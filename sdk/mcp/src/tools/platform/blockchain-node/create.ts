...
export const platformBlockchainNodeCreate = (server: McpServer, env: Partial<DotEnv>, pat: string) => {
  ...
  server.tool(
    ...
    async ({ applicationUniqueName, blockchainNetworkUniqueName, name, type, size, nodeType, provider, region }) => {
        ...
        content: [
          {
            ...
            description: `Created blockchain node: ${name} in network: ${blockchainNetworkUniqueName}`,
