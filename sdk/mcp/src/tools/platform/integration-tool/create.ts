...
export const platformIntegrationToolCreate = (server: McpServer, env: Partial<DotEnv>, pat: string) => {
  ...
  server.tool(
    "platform-integration-tool-create",
    {
      ...
      integrationType: z
        .enum(["CHAINLINK", "HASURA", "HA_HASURA", "INTEGRATION_STUDIO"])
        .describe("Type of integration"),
    },
    async ({ applicationUniqueName, name, type, size, provider, region, integrationType }) => {
        ...
        content: [
          {
            ...
            description: `Created integration tool: ${name} in application: ${applicationUniqueName}`,
