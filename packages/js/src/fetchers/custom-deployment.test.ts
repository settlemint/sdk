...
describe("Custom Deployment Fetchers", () => {
  ...
  describe("customdeploymentList", () => {
    test("should return a list of custom deployments", async () => {
      const mockResponse = {
        customDeployments: {
          items: [
            {
      ...
      const list = customdeploymentList(mockGqlClient, mockOptions);
      const result = await list("123e4567-e89b-12d3-a456-426614174002");

      expect(result).toEqual(mockResponse.customDeployments.items);
  ...
  describe("customdeploymentRead", () => {
    test("should return a specific custom deployment", async () => {
      const mockResponse = {
        customDeployment: {
      ...
      const read = customdeploymentRead(mockGqlClient, mockOptions);
      const result = await read("123e4567-e89b-12d3-a456-426614174003");

      expect(result).toEqual(mockResponse.customDeployment);
      expect(mockGqlClient.request).toHaveBeenCalledTimes(1);
      expect(mockGqlClient.request).toHaveBeenCalledWith(expect.anything(), {
        id: "123e4567-e89b-12d3-a456-426614174003",
      });
    ...
    test("should throw an error for invalid custom deployment ID", async () => {
      const read = customdeploymentRead(mockGqlClient, mockOptions);
